...
internal class BrokenVessel : TemplateBase
{
    ...
    {
        prefab = Load.Preload(preloadedObjects, "GG_Broken_Vessel", "Infected Knight");
    }
    ...
    protected override void RewriteInstance(GameObject gameObject, PlaceConfig properties)
    {
        ...
        fsm.InsertCustomAction("Init", () =>
        {
            ...
            var shakeTokenControl = gameObject.LocateMyFSM("Shake Token Control");
            var maxHP = gameObject.GetComponent<HealthManager>().hp;
            var token1 = maxHP * 0.37f;
            var token2 = maxHP * 0.22f;
            var token3 = maxHP * 0.11f;
            shakeTokenControl.FsmVariables.GetFsmFloat("Token 1").Value = token1;
            shakeTokenControl.FsmVariables.GetFsmFloat("Token 2").Value = token2;
            shakeTokenControl.FsmVariables.GetFsmFloat("Token 3").Value = token3;
            ...
            spawnBalloon.AccessIntVariable("HP Threshold").Value = (int)(maxHP * 0.42f);
        ...
        fsm.RemoveTransition("Init", "ACTIVE");
        fsm.AddTransition("Init", "ACTIVE", "Idle");
