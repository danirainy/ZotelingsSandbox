...
internal class FailedChampion : TemplateBase
{
    ...
    {
        prefab = Load.Preload(preloadedObjects, "GG_Failed_Champion", "False Knight Dream");
        rocksPrefab = Load.Preload(preloadedObjects, "GG_Failed_Champion", "FK Barrel Summon Dream");
    }
    ...
    protected override void RewriteInstance(GameObject gameObject, PlaceConfig properties)
    {
        ...
        fsm.AddTransition(Deploy.Common.SleepingStateName, "FINISHED", "Wait for Hero");
        fsm.RemoveAction("Wait for Hero", 0);
        fsm.AddCustomAction("Wait for Hero", () =>
        {
        ...
        fsm.RemoveTransition("Wait for Hero", "FINISHED");
        fsm.AddTransition("Wait for Hero", "FINISHED", "Idle");
        fsm.RemoveAction("Check", 0);
        fsm.AddCustomAction("Check", () =>
        {
            fsm.SendEvent("FINISHED");
        });
        fsm.RemoveTransition("Check Direction", "CANCEL");
        fsm.RemoveAction("Check GG", 1);
        fsm.AddCustomAction("Check GG", () =>
        {
            fsm.SendEvent("GG BOSS");
        });
        fsm.InsertCustomAction("Dream Return", () =>
        {
            fsm.SendEvent("GG BOSS");
        }, 0);
        fsm.RemoveAction("Blow", 1);
        fsm.RemoveAction("Cough", 3);
